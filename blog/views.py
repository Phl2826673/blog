...
class ArticleDetail(DetailView):
    ...
    def comment_sort(self, comments):  # 评论排序函数
        ...
        for comment in comments:  # 遍历所有评论
            if comment.reply_id == None:  # 如果没有回复目标
...
def sites(request):
    ...
    import datetime
    publish = datetime.datetime.now().strftime('%Y-%m-%d')
    article_id = pk
    reply_id = 2
    sql = "insert into blog_comment (name,email,content,publish,article_id,reply_id) values(%s,%s,%s,%s,%s,%s)"
    aa = db.update(sql, params=(name,email,content,publish,article_id, None))
    # a2 = db.update(sql, params=(name,email,content,publish,article_id, None))
